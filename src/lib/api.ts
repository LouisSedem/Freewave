...
// Search YouTube via server proxy (parses ytInitialData from YouTube HTML)
...
// Combined search: YouTube (full tracks) + iTunes metadata, smart-merged
export async function searchAll(query: string): Promise<Track[]> {
  ...
  // Order: upgraded iTunes (best quality) → unmatched YouTube → pure iTunes (30s fallback)
